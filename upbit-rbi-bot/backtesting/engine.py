...
전략 신호 + 헌장의 TP/SL/시간손절을 그대로 적용해 실전과 동일 규칙으로 검증한다.
"""
from __future__ import annotations

import pandas as pd

from config.charter import FEE_ROUNDTRIP, TIME_STOP_BARS, STRATEGY_SPECS
...
def run(strategy: BaseStrategy, df: pd.DataFrame, warmup: int = 30) -> BacktestResult:
    ...
    for i in range(warmup, len(df)):
        ...
        exit_now = False
        if gross >= spec.take_profit:          # 익절
            exit_now = True
        elif gross <= -spec.stop_loss:         # 손절
            exit_now = True
