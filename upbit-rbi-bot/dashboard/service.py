...
class BotService:
    ...
    def expectations(self) -> dict:
        ...
        v2.3에서 15분봉 진입선(3→7)·게이트(1.0%→0.83%)를 바꿨다가 **v3.0에서 취소**했다.
        따라서 아래 수치(개정 전 파라미터의 홀드아웃 측정치)는 현재 가동 설정과 **일치한다**.
        취소 근거: 2년 재측정에서 개정 전이 모든 조합에서 우세(6종목 +0.457% vs +0.130%,
        전체 종목 +0.243% vs −0.026%) — backtesting/research/README.md.
        """
        return {
