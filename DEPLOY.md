...
## 헌장 v1.4 배포 시 주의 (전략 교체 + 스프레드 필터 + 15분봉 병행)

v1.3부터 실거래에는 `.env` 에 **두 값**이 모두 필요하다 (헌장 §9.7):

```
DRY_RUN=false
LIVE_CHARTER_ACK=v1.4     # ← 없으면 배포해도 자동으로 '모의 모드'로 강제된다
```

- 기존 서버 `.env` 에는 `LIVE_CHARTER_ACK` 이 없으므로, **이 버전을 배포하면 봇은 모의 모드로 전환된다**(주문 미전송). 부팅 시 텔레그램으로 사유를 알린다.
- 이는 의도된 동작이다: v1.3~v1.4는 전략을 macd/rsi/cvd → **rsi2(5분) + rsi2_15m(15분)** 으로 교체했으므로, 규칙 개정마다 명시적 승인을 요구한다.
...
권고 절차는 모의 2~4주 후, 실제 체결 비용이 백테스트 가정(수수료 0.1% + 슬리피지 0.05~0.10%) 이내임을 확인한 다음 `LIVE_CHARTER_ACK=v1.4` 을 추가하고 재시작하는 것이다.
...
## 실전(LIVE) 전환 체크리스트
- [ ] `.env` 의 `DRY_RUN=false`
- [ ] `.env` 의 `LIVE_CHARTER_ACK=v1.4` (헌장 §9.7 — 없으면 모의로 강제)
