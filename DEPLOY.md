...
```
DRY_RUN=false
LIVE_CHARTER_ACK=v1.5     # ← 없으면 배포해도 자동으로 '모의 모드'로 강제된다
```
...
권고 절차는 모의 2~4주 후, 실제 체결 비용이 백테스트 가정(수수료 0.1% + 슬리피지 0.05~0.10%) 이내임을 확인한 다음 `LIVE_CHARTER_ACK=v1.5` 을 추가하고 재시작하는 것이다.
...
## 실전(LIVE) 전환 체크리스트
- [ ] `.env` 의 `DRY_RUN=false`
- [ ] `.env` 의 `LIVE_CHARTER_ACK=v1.5` (헌장 §9.7 — 없으면 모의로 강제)
